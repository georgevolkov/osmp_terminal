...
function processProvider(sNumber) {
    ...
    var sTxt = getText("cellular_successful_notice_beeline", sLocale);
    var sNoPrv = getText("cellular_unsuccessful_notice", sLocale);
    var sBadFormat = getText("cellular_bad_format_beeline", sLocale);
    var prov = null;
    ...
    if (bIsFoundProvider) {
        ...
        if (nPrvId == "11823") {
            $("num_info_txt2_finca").innerHTML = sTxt;
            $("num_info_txt2_finca2").innerHTML = sTxt;
        }
        $('num_info_txt2').className = "";
        if (!testProviderByMask(prov.providerId, sNumber)) {
            sTxt = sBadFormat;
            $("btn_forward").style.display = 'none';
            $("num_info_txt2").className = "num_info_txt2";
            }
    } else {
        nPrvId = 0;
        sPrv = sPrvImage = "";
