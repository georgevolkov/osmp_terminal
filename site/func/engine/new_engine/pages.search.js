...
(
	CPage,
	{
	    ctor: function(oParent, sInstance, sCtrlPlace) {
	        ...
	        this._applySearchString = $delegate(this, function(){                    
	        	$("SearchProvidersString").value = this._sSearchString;
                        //searchProviders из файла search_providers.js
                        var oProviders = searchProviders(this._sSearchString);
                        var sProvidersString = convertProvidersArrayToString(oProviders);                        
                        $("SearchResult").value = sProvidersString;
			this._updatePrvList();
	        });
	    ...
	    _createContent: function(sPlace) {
	        $(sPlace).vAlign = "top";
	        $(sPlace).innerHTML =
                '<div style="width: 1260px; height: 121px; margin-top: 28px;">' +
                ...
                '                    <div style="height: 40px; text-align: left; color: #fff; font-size: 22px; padding-top: 5px;">'+getText("enter_provider_name", this._sLocale)+'</div>' +
                '                  </td>' +
                '               </tr>' +
                '            </table>' +
                '          </td>' +
	    ...
	    _drawBackspaceBtn: function(sPlace) {
                //var oBtn = new CBtmMenuButton(this, "bs_btn", sPlace, "delete_empty.gif", "delete_press.gif", {id: "button_delete", locale: this._sLocale});
	        var oBtn = new CImageButton(this, "bs_btn", sPlace, "BACKSPACE", "./img/ui/navigation/delete_empty.gif", "./img/ui/navigation/delete_empty.gif", {id: "button_delete", locale: this._sLocale});
	    ...
	    _drawProviders: function(sPlace) {
	        ...
	        if(this._aProviders.length)
	        {
	            ...
	            if (!this._sSearchString.length) {                        
	                sContent = '<div style="margin: 0; text-align: center; padding-top: 100px; font-size: 50px; font-weight: normal; color: #2268c6;">'+getText("enter_first_letter", this._sLocale)+'</div>';
	            }
	    ...
	    _createBottomMenu: function(sPlace) {
	        ...
	        this._oRightBtmBtn = new CBtmMenuButton(this, "right_btn", "btn_right", "next_empty.gif", "next_press.gif", {id: "button_next", locale: this._sLocale});
	    ...
	    _getProviderPlaces: function() {
            ...
            _formOldProvider: function(iProvId) {
                var oProvider = {};
                oProvider.prv_id = value[iProvId]["prv_id"];
                oProvider.prv_name = value[iProvId]["prv_name"];
                return oProvider;
            },

            _saveProviderInCookie: function(eargs) {
                document.cookie = "prv_id=" + value[eargs.value]["prv_id"];
                document.cookie="prv_folder="+value[eargs.value]["prv_folder"];
                document.cookie="prv_value="+eargs.value;
                //document.cookie="prv_alert="+value[eargs.value]["prv_alert"];
                document.cookie="prv_alert=" + null;
                document.cookie="prv_image="+value[eargs.value]["prv_image"];
                document.cookie="prv_group="+value[eargs.value]["prv_group"];
                document.cookie="prv_mask="+value[eargs.value]["prv_mask"];
                document.cookie="prv_name="+getProviderText(eargs.value, this._sLocale); // value[eargs.value]["prv_name"];
                storage.put("prv_invitation", value[eargs.value]["prv_invitation"], true);
                storage.put("prv_online_check", value[eargs.value]["prv_online_check"], true);
            },

	    _providerClick: function(sender, eargs) {                                
	        //this._getProvider(eargs.value);
                this._saveProviderInCookie(eargs);
                var oProv = this._formOldProvider(eargs.value);
                this._postData(value[eargs.value]["prv_page"], oProv);
	    },

	    _getProvider: function(sId) {
	        if($is(sId, String)) getScript('./config/' + sId + '.js', $delegate(this, this._applyProvider));
	    },
	    ...
	    _postData: function(sUrl, oPrv) {
	        var oForm = new CForm(this, "oForm", sUrl, "ff", "post");
	        if (!$isNoU(oPrv)) {
	            if (!$isNoU(oPrv["prv_id"])) {
	                oForm.put("prv_id", oPrv["prv_id"]);
	            }
	            if (!$isNoU(oPrv["prv_name"])) {
	                oForm.put("prv_name", oPrv.prv_name);
	            }
