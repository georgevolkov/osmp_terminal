...
(
	CButton,
	{
	    ctor: function(oParent, sInstance, sCtrlPlace, sEnabledImg, sDisabledImg, oLocaleObject) {
	        ...
	        this._sBaseFolder = "./img/ui/navigation/";
                this._oLocaleObject = oLocaleObject;
	    ...
	    _paint: function() {
	        ...
	        if (!Object.isNullOrUndefined(oDiv)) {
	            ...
	            //oDiv.innerHTML = "<img src=\"" + this._sBaseFolder + this._sEnabledImg + "\" />";
                    var sImageUrl = this._sBaseFolder + this._sEnabledImg;
                    oDiv.innerHTML = '<div style="margin:0;padding:0; width:301px; height:121px; background:url('+sImageUrl+') no-repeat;"><div id="'+this._oLocaleObject.id+'"></div></div>';
                    getTextById(this._oLocaleObject.id, this._oLocaleObject.locale);
	            oDiv.onclick = null;
	    ...
	    _paint: function () {
	        ...
	        if (!$isNoU(oDiv)) {
	            if (this._sLogo.length) {
	                oDiv.innerHTML = '<div style="background: url(\'./img/ui_item/top8/' + this._sLogo + '\') no-repeat"></div>';
	                //oDiv.innerHTML = '<div></div>';
	            }
	            else {
	                oDiv.innerHTML = [
			        '<div style="background: url(\'./img/ui/grp_panel/bg_tp.gif\') no-repeat">',
		            	'<table width="100%" height="54" cellpadding="0" cellspacing="0" border="0" style="margin-top: 10px;"><tr>',
...
(
	CValueButton,
	{
	    ctor: function(oParent, sInstance, sCtrlPlace, sValue, sActivePic, sInactivePic, oLocaleObject) {
                this._oLocaleObject = oLocaleObject;CBtmMenuButton
	    ...
	    _paint: function() {
	        ...
	        if (!Object.isNullOrUndefined(oDiv)) {
	            // oDiv.innerHTML = "<img alt=\"" + this.getInstance() + "\" src=\"" + this._sInactivePic + "\" onMouseOver=\"this.src='" + this._sActivePic + "'\" onMouseOut=\"this.src='" + this._sInactivePic + "'\">";
                    var sImageUrl = this._sActivePic;
                    oDiv.innerHTML = '<div style="width:301px; height:100%; background:url('+sImageUrl+') no-repeat;"><div id="'+this._oLocaleObject.id+'"></div></div>';
                    getTextById(this._oLocaleObject.id, this._oLocaleObject.locale);
