	        ...
	        for (var i = 0; i < this._aFldObjects.length; i++) {
	            ...
	            if (!$isNoU(this._aFields[i].disp_type)) {
	                ...
	                if (this._aFields[i].disp_type.indexOf('extra') != -1)
	                    this._oParent._oPostData["_extra_" + this._aFields[i].name] = $(this._aFields[i].disp_name).value;
	                if (this._aFields[i].disp_type.indexOf('receipt') != -1)
	                    if (!$isNoU(this._aFields[i].disp_desc)) 
	                        this._oParent._oPostData["_receipt_" + this._aFields[i].disp_desc] = $(this._aFields[i].disp_name).value;
	                    else
	                        this._oParent._oPostData["_receipt_"] = $(this._aFields[i].disp_name).value;
	            }
	        }
	    }
    }
...
(
	CInputTab,
	{
	    ...
	    _prepareTemplate: function () {
	        ...
	        {
	        	providerElement = '<div style="background-image: url(./'+getFileName(this._sLogo)+')"></div>';
	        }
	        else
	        {
...
{
	return '<div class="provider-button" id="providerButton"><div style="background-image: url(\'./' + image + '\')"></div><p>' + title + '</p></div>';
};
