	        ...
	        if (this._aChunks.length) {
	            ...
	            for (var i = 0; i < this._aChunks.length; i++) {
	                if (this._sType == "SINGLE_FRAMED" || this._sType == "SINGLE_STATIC") {
	                    sContent +=
	                    '<td class="field_chunk" id="' + this._sInstance + '_chunk_' + i + '_place" align="center" style="' + ((this._sType == "SINGLE_FRAMED") ? 'background: url(\'./img/ui/field/c.gif\') repeat-x;' : 'background: #fff;') + '">' + this._aChunks[i].value + '</td>';
	                }
	                else if (this._sType == "SINGLE_DYNAMIC") {
	                    sContent +=
	                    '<td class="field_chunk" align="center" style="background: url(\'./img/ui/field/c.gif\') repeat-x;"><div id="' + this._sInstance + '_chunk_' + i + '_place" style="width: ' + ((this._aChunks[i].maxLength * this._nWidthCoefficient)+35) + 'px; overflow: hidden;"></div></td>';
	                }
	...
	CControl,
	{
	    ...
	    _paint: function() {
	        ...
	        if (this._aBlockCfgs.length) {
	            sContent += this._sHeader.toFormat("color: #26b; font-size: 22px; font-weight: normal;") + "<br />";
	            sContent +=
	            ...
	            sContent +=
                    ...
                    '</table>' +
                    '</div>';
	            sContent += this._sFooter.toFormat("color: #FF4800;");
	            insertContent(this.getPlaceId(), sContent);
	            for (var i = 0; i < this._aBlockCfgs.length; i++) {
...
(
	CField,
	{
	    ...
	    _paint: function() {
	        ...
	        var sContent = '';
	        if (this._aBlockCfgs.length) {
	            sContent += this._sHeader.toFormat("color: #26b; font-size: 18px; font-weight: normal;") + "<br />";
	            sContent +=
	            ...
	            if (nIdx > -1) {
	                ...
	                if (this._isCorrectChunk(sTemp)) {
	                    ...
	                    aResult.push({ type: "DYNAMIC",
	                        regExp: sRegExp,
	                        minLength: nMinLength,
	                        maxLength: (nMaxLength < 45 ? nMaxLength : 45)
	                    });
	                }
	                sBlock = sBlock.substr(nIdx + 2);
	            }
